...
export function TicketList({ tickets, dense, onSelect, onOpenCliente, onToggleRead }: { tickets: Ticket[]; dense?: boolean; onSelect: (id: string) => void; onOpenCliente?: (kind: ClienteKind, id: string) => void; onToggleRead?: (id: string, read: boolean) => void }) {
  return (
        ...
        {tickets.map((t) => (
          <li key={t.id}>
            <button onClick={() => onSelect(t.id)} className={`w-full text-left flex items-center gap-3 hover:bg-slate-50 ${dense ? 'px-4 py-1.5' : 'px-4 py-3'}`}>
              ...
              <ReadToggle read={t.read} onToggle={(r) => onToggleRead?.(t.id, r)} className="text-[16px] shrink-0" />
