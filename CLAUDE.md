...
| — (regla de mutación 4) | **IV-10.** La línea base de citas `ruta:línea` ya rotas que el detector de `hook-citas-pre-push` genera en `apps/desk/server/citas/lineaBase.jsonl`, para imponerse en `pre-push`: **7** entradas, 7 claves (documento, cita) distintas, desde el 2026-09-15 en `334142b`, que reparó el bloque B3 (6 extremo inicial en línea vacía y 1 extremo inicial fuera de rango). *(Antes decía, y se conserva como caso C: «**13** entradas, 13 claves (documento, cita) distintas, desde el 2026-09-15 en `e982672`, que reparó el bloque B2 (11 extremo inicial en línea vacía, 1 extremo inicial fuera de rango y 1 ambigua rota en todas sus candidatas)»; antes de B2: «**23** entradas, 22 claves (documento, cita) distintas, desde el 2026-09-15 en `37d6e71`, que reparó el bloque B1 (15 extremo inicial en línea vacía, 4 extremo inicial fuera de rango, 1 extremo final fuera de rango, 2 fichero inexistente y 1 ambigua rota en todas sus candidatas)»; y antes de B1: «**37** entradas, 28 claves (documento, cita) distintas, medida el 2026-09-14 sobre `73a9acb` con el detector definitivo (27 extremo inicial en línea vacía, 5 extremo inicial fuera de rango, 1 extremo final fuera de rango, 3 fichero inexistente y 1 ambigua rota en todas sus candidatas)».)* La base sólo encoge y no crece desde el hook: una entrada nueva exige editarla a mano. Saltarse el hook con `--no-verify` **no** es una salida legítima de un bloqueo; las dos legítimas son reparar la cita o añadirla a mano a la base (mitigación de R-4 de `hook-citas-pre-push`) | **REPARACIÓN DE LA BASE POR BLOQUES, decidido el 2026-09-15** (P.2 de `hook-citas-pre-push`): la repara el terminal como trabajo documental directo y Gerencia aprueba por bloques; IV-10 se cierra cuando la base llegue a 0. *(Antes decía, y se conserva como caso C: «**SIN ASIGNAR, decidido explícitamente, no por omisión** (Q4 de `hook-citas-pre-push`): «la base no encoge hasta que Gerencia asigne quién la repara»» — decidido el 2026-09-15.)* |
