# Publicar el motor `@ambientalia/zoho-sync` (Enfoque C) — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax.

**Goal:** Hacer `@ambientalia/shared` y `@ambientalia/zoho-sync` publicables a GitHub Packages (compilados + versionados), publicar v1.0.0, sin cambiar cómo el monorepo los consume (sigue en fuente).
...
- `packages/zoho-sync/package.json`: `{ private:true, version:0.0.0, exports:{ "./*":"./src/*.ts" }, dependencies:{ "@ambientalia/shared":"*" } }`. 3 `.sql`: `src/db/schema.sql`, `src/booksHub/schema-books.sql`, `src/crmHub/schema-crm.sql` (leídos vía `fileURLToPath(import.meta.url)`).
- `tsconfig.base.json`: `noEmit:true`, `moduleResolution:"Bundler"`, `paths:{ "@ambientalia/shared":["./packages/shared/src/index.ts"], "@ambientalia/zoho-sync/*":["./packages/zoho-sync/src/*"] }`.
- `vitest.config.ts`: alias `@ambientalia/shared` → `packages/shared/src/index.ts` (no hay alias para zoho-sync; hoy resuelve por exports=src).
- `apps/desk/vite.config.ts`: alias `@ambientalia/shared` → `packages/shared/src/index.ts`.
...
**Orden de tareas:** T1 = `shared` publicable + **cableado global del monorepo** (tsx conditions, aliases) — es el canario que prueba todo el enfoque en el paquete más simple. T2 = `zoho-sync` publicable (+ copia `.sql`). T3 = infra de publicación + doc + dry-run. T4 = publish v1.0.0 (manual) + memoria.

---

## Task 1: `@ambientalia/shared` publicable + cableado global del monorepo
...
// ...
  resolve: {
    alias: [
      { find: /^@ambientalia\/shared$/, replacement: fileURLToPath(new URL('./packages/shared/src/index.ts', import.meta.url)) },
      { find: /^@ambientalia\/shared\/(.*)$/, replacement: fileURLToPath(new URL('./packages/shared/src/', import.meta.url)) + '$1.ts' },
      { find: /^@ambientalia\/zoho-sync\/(.*)$/, replacement: fileURLToPath(new URL('./packages/zoho-sync/src/', import.meta.url)) + '$1.ts' },
    ],
  },
```
...
- [ ] **Step 6: Verificar el monorepo intacto (consumiendo fuente).**
  ...
  - **Smoke runtime (resolución a fuente vía tsx conditions):** `node -e "process.exit(0)"` no aplica; en su lugar: `npx tsx --conditions=development -e "import('@ambientalia/shared').then(m=>{if(!m) throw new Error('no shared'); console.log('shared OK via src')})"` → imprime `shared OK via src` (resuelve a `src/index.ts`, NO a dist que no existe).
...
- [ ] **Step 7: Verificar que el build de `shared` emite dist (para publicar luego).**
  - Run: `npm --workspace @ambientalia/shared run build` → crea `packages/shared/dist/` con `index.js` + `index.d.ts` + el resto de módulos.
...
## Task 2: `@ambientalia/zoho-sync` publicable (+ copia de `.sql` a dist)
...
```json
{
  ...
  "publishConfig": { "registry": "https://npm.pkg.github.com" },
  "dependencies": { "@ambientalia/shared": "^1.0.0" }
}
```
(NOTA: `@ambientalia/shared: "^1.0.0"` en vez de `"*"` — el consumidor externo traerá shared 1.x. En el monorepo, npm sigue resolviendo el workspace local por nombre.)

- [ ] **Step 2: `packages/zoho-sync/tsconfig.build.json`** (igual que shared; `paths:{}` → `@ambientalia/shared` resuelve por node_modules a su dist ya compilado):
```json
{
  "extends": "../../tsconfig.base.json",
  "compilerOptions": {
...
- [ ] **Step 4: Verificar monorepo intacto (consume fuente).**
  ...
  - Run: `npx eslint .` → 0 errores.
  - **Smoke runtime zoho-sync vía fuente:** `npx tsx --conditions=development -e "import('@ambientalia/zoho-sync/db/migrate').then(m=>{if(!m.migrate) throw new Error('no migrate'); console.log('zoho-sync OK via src')})"` → imprime `zoho-sync OK via src`.

- [ ] **Step 5: Verificar el build completo (dist con JS + d.ts + sql).** Orden: shared primero (zoho-sync lo necesita compilado).
  - Run: `npm --workspace @ambientalia/shared run build`
  - Run: `npm --workspace @ambientalia/zoho-sync run build` → `tsc` emite + `copy-sql` copia 3 `.sql`.
  ...
  - **Smoke del artefacto compilado:** `npx tsx -e "import('@ambientalia/zoho-sync/db/migrate').then(m=>console.log('dist resolve:', typeof m.migrate))"` (SIN `--conditions` → resuelve a `dist/db/migrate.js`) → imprime `dist resolve: function`. Confirma que el `default`/dist funciona para externos.
...
```
@ambientalia:registry=https://npm.pkg.github.com
...
```markdown
# Publicar `@ambientalia/shared` y `@ambientalia/zoho-sync` (GitHub Packages)

## Publicar una versión
1. Bump de versión en ambos `package.json` (mismo número, p. ej. 1.0.1). `zoho-sync` depende de `@ambientalia/shared: "^1.x"`.
...
3. Build + publish (shared PRIMERO, por la dependencia):
   ```
   npm --workspace @ambientalia/shared run build
   npm publish --workspace @ambientalia/shared
   npm --workspace @ambientalia/zoho-sync run build
   npm publish --workspace @ambientalia/zoho-sync
   ```
...
2. `npm i @ambientalia/zoho-sync@^1` (trae también `@ambientalia/shared`).
3. Uso: la lib expone el motor (clientes Zoho, migrate, sync). La app aporta su config (env Zoho) y un `pg` Pool.
   Ejemplo: `import { createPoolFromUrl } from '@ambientalia/zoho-sync/db/pool'`.
```
...
- [ ] **Step 4: Dry-run de empaquetado (sin publicar).**
  - Run: `npm --workspace @ambientalia/shared run build && npm --workspace @ambientalia/zoho-sync run build`
  - Run: `npm pack --workspace @ambientalia/shared --dry-run` → la lista de archivos incluye SOLO `dist/**` + `package.json` (no `src/`).
  - Run: `npm pack --workspace @ambientalia/zoho-sync --dry-run` → incluye `dist/**` con `.js`, `.d.ts` y los 3 `.sql`; NO incluye `src/`.
...
npm publish --workspace @ambientalia/zoho-sync
```
Verificar en GitHub → repo → Packages que aparecen `@ambientalia/shared@1.0.0` y `@ambientalia/zoho-sync@1.0.0`.

- [ ] **Step 3: (Opcional) Probar consumo externo.** En un dir temporal fuera del monorepo: `.npmrc` (read:packages) + `npm i @ambientalia/zoho-sync@^1` + un `import('@ambientalia/zoho-sync/db/migrate')` → resuelve a `dist`.
...
- [ ] **Step 5: Actualizar memoria.** En `zoho-hub-arquitectura.md`: motor `@ambientalia/zoho-sync` + `shared` **publicables/publicados** en GitHub Packages (v1.0.0); monorepo consume fuente vía `--conditions=development` + aliases; runbook en `docs/publicar-paquetes.md`. (Enfoque C; A/B descartados.)
