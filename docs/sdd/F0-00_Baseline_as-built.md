...
2. **La corrección C1 sigue abierta y no tiene ninguna prueba.** `apps/desk/server/transitionExec.ts:63-70` procesa la rama del checkbox con `continue` antes del chequeo de campo obligatorio. La única prueba de obligatorios (`apps/desk/server/transitionExec.test.ts:82-86` en `a3a8f03`; cerrado después: la prueba, F0-04 en b5deda0 y 3d7be3f; el defecto C1, F1A-01 en ec0ed1f) usa un campo de texto, nunca una casilla. Bajo `strict_tdd`, **F1A-01 no puede arrancar en rojo**: hay que escribir la prueba que falla antes de tocar el código.
...
(desplazada desde entonces; `foto-solo-con-novedad`/F1B-04 la corrió a `:17` sobre `a0a2935`, el
inventario entero es del commit auditado y no se renumera), `EliminarTicket.tsx:14`.
...
**e.6 — El defecto C1 no tiene ninguna prueba, ni que lo fije ni que lo denuncie.** La prueba de obligatorios `apps/desk/server/transitionExec.test.ts:82-86` en `a3a8f03` (cerrado después: prueba de C1 en F0-04, b5deda0; arreglo en F1A-01, ec0ed1f) usa `ingreso_a_servicio` y el campo de texto «Código Servicio», nunca una casilla. → **Bloquea F1A-01 bajo `strict_tdd`.**
...
**e.9 — `estadoPorRemision.ts` y `ticketService.ts` no tienen fichero de prueba propio.** No existe `estadoPorRemision.test.ts`; `sincronizarEstadoPorRemision` sólo se ejercita de refilón desde `app.test.ts:2421` vía `apps/desk/server/routes/remision.ts:332`, `:342` y `:370` (reapuntadas por `foto-solo-con-novedad`/F1B-04, que insertó 7 líneas en `:283-289`; cita válida en `a0a2935` como `:325`, `:335` y `:363`). La guarda de no-cruce (`estadoPorRemision.ts:41`), que es la regla invariable 7, no tiene prueba que la nombre. `executeTransition` (`ticketService.ts:74-181`) y `createManagedTicket` (`:20-71`) sólo se prueban por HTTP.
...
|---|---|---|---|
| **H.4 · M7 — Analítica y KPIs** | «Parcial: **trece indicadores se calculan hoy** sobre la tabla de tickets (Anexo G)» | Los **11 indicadores calculados del Anexo G.6** (cols. 47, 48, 49, 50, 51, 53, 54, 56, 57, 58, 59) **no tienen ninguna fórmula** en `apps/desk` ni en `packages/*`; grep exhaustivo de sus nombres sólo devuelve `docs/analisis-tickets/*`. Sólo están almacenadas las 16 fechas de hito de G.5 y `dias_entrega` (`packages/zoho-sync/src/db/rows.ts:83-123` en `a3a8f03`; superado en parte por C9, e8c5e90: añadió `fecha_aviso_cliente` a esa lista y calcula los tres bodegajes de M1.10 en `packages/shared/src/bodegaje.ts`, el de entrada es la col. 56 del Anexo G.6 (maestro R08.1, línea 1678); el resto de indicadores de G.6 sigue sin fórmula y la capacidad `kpis` es de Fase 2) | Corregir la fila M7: si esos indicadores se calculan, es **fuera** de Desk 2.0 (Zoho Analytics u hoja de cálculo). Punto abierto nuevo: dónde vive hoy el cálculo y quién lo hereda en la Fase 2 |
...
| **H.4 · M11 — Arquitectura** | Describe una sola conexión | Topología real de **dos bases** (`zoho-hub-db` + `desk-db`) con replicación lógica de sólo 4 tablas, y **dos ciclos independientes** de lectura directa contra Zoho Desk (`apps/desk/server/index.ts:81-90` y `apps/hub-sync/src/hubSync.ts:75-101`) | Ampliar la fila M11 con la topología y el doble consumo de cuota; punto abierto nuevo sobre si se consolida en un solo lector |
...
| **Anexo G · col 55** | «Calificación de satisfacción … sólo para reporte trimestral» | No existe en `PROMOTED_COLUMNS` (`packages/zoho-sync/src/db/rows.ts:83-123` en `a3a8f03`) ni en `schema.sql`; grep sin resultados en `apps/desk` ni `packages/*` | Anotar en H.5 (fila «Satisfacción del cliente») que el dato **no está replicado** en Desk 2.0 |
...
| M-15 paginación en la UI de gestión de equipos | POSIBLE OBSOLETO, **sin confirmar** | `apps/desk/src/components/EquiposAdmin.tsx` (estado `page`); `apps/desk/server/db/equipos.ts:142-152` en `a3a8f03` (acepta `limit`/`offset`) | Sugiere que se implementó tras escribirse la entrada; **no se verificó** el wrapper `listEquiposManage` de `api/client.ts` |
...
6. **`hoja-de-vida-equipo-design.md` (06-05) — PARCIAL.** Diseñaba la cronología del equipo sólo desde `ticket_transitions`. `apps/desk/server/db/equipos.ts:261-292` en `a3a8f03` fusiona además `ticket_history` («Y las etapas que NO pasaron por Desk, reconstruidas desde la historia de Zoho»). **Evolución posterior sin diseño que la documente.**
...
**Lo que sí está en el repo es el disparador.** `apps/desk/server/routes/remision.ts:309` (reapuntada por
`foto-solo-con-novedad`/F1B-04; cita válida en `a0a2935` como `:302`) llama a `dispararRemision` (`apps/desk/server/remisionWebhook.ts:75-108`), que hace `POST` al webhook de n8n (`config.remisionWebhookUrl`) con el payload que arma `buildRemisionPayload` (`remisionWebhook.ts:31-66`). Ese `POST` dispara las siete ramas paralelas del flujo, una de las cuales es `BBDD remisiones_entrada`.
...
|---|---|---|
| **Anexo D · 33 (C1)** | La mitad técnica está confirmada: la guarda sigue abierta y, además, **no existe ninguna prueba** que la cubra. La mitad de negocio (si «Liberación del ticket sin facturar» sigue siendo checkbox o pasa a campo exigible, y si se restringe a un cargo) sigue siendo de Gerencia | `apps/desk/server/transitionExec.ts:63-70`; `packages/shared/src/transitions.ts:247`; `apps/desk/server/transitionExec.test.ts:82-86` en `a3a8f03` (mitad técnica cerrada después: F0-04 en b5deda0; F1A-01 en ec0ed1f) |
...
3. **Tres diseños hay que actualizarlos antes de destilarlos.** `hoja-de-vida-equipo-design.md` no documenta la fusión con `ticket_history` que el código sí hace (`apps/desk/server/db/equipos.ts:261-292` en `a3a8f03`); `articulos-por-modelo-design.md` declara tres clases y el código tiene otras tres distintas (`packages/shared/src/types.ts:239`); `subsistema-i-sync-zoho-books-design.md` diseña tablas y el código tiene vistas (`schema.sql:169,176`).
...
| **Prueba en rojo de C1**, antes de F1A-01 | `apps/desk/server/transitionExec.ts:63-70`; `transitionExec.test.ts:82-86` en `a3a8f03` usa campo de texto (hecho después: prueba en rojo en F0-04, b5deda0; verde con F1A-01, ec0ed1f) |
...
El plan la marca con gate «Ninguno» y tamaño XS. Es correcto en cuanto al código —el arreglo es una línea— pero **bajo `strict_tdd` la tanda no puede arrancar en rojo**: no existe ninguna prueba que ejercite el camino del checkbox obligatorio. Evidencia: `apps/desk/server/transitionExec.ts:63-70` (la rama sale antes del chequeo), `packages/shared/src/transitions.ts:247` (único caso real), `apps/desk/server/transitionExec.test.ts:82-86` en `a3a8f03` (la prueba de obligatorios usa el campo de texto «Código Servicio», nunca una casilla; cerrado después: F0-04 en b5deda0, F1A-01 en ec0ed1f). Si no se escribe primero la prueba que falla, el arreglo se dará por bueno sin evidencia. **Dependencia real: F0-04 (o la propia F1A-01 empezando por la prueba).**
