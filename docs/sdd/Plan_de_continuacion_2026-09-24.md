...
| **⚠️ Encuesta de satisfacción** | Gerencia | Entra en el criterio junto a los indicadores, pero `config.yaml:2522` la declara «alcance que la pregunta no contenía» y `:2630` dice que es **pieza sin fila hoy**. Cruza con el correo propio |
...
| **`equipo-nuevo-alta-en-ticket`** | Gerencia | Destino *propuesto* F1B-06, no escrito (`ENTRADA.md:1089` en `71499ba`; cerrada el 24/09, fila propia). Sin decisión no entra en ninguna fila |
| **`edicion-datos-comerciales-equipo`** | Gerencia | **Sin destino.** Es además decisión de permiso (`ENTRADA.md:1097` en `71499ba`; cerrada el 24/09, misma fila que E-070) |
| **`mapa-en-la-app`** | Gerencia | **Sin destino.** F1A-06 está archivada y F1B-09 es auditoría: ninguna fila viva lo contiene (`ENTRADA.md:1105` en `71499ba`; cerrada el 24/09, fila propia) |
...
| Tarea | Dueño | Fecha |
|---|---|---|
| **Los tres recuentos contra producción** (histórico C1, liberaciones sin factura y el tercero) | Alfonso | **viernes 25/09/2026** — decidido en `decision/p64b-quien-ejecuta` (`config.yaml:2613-2628`). Claude Code prepara antes **un único fichero con las consultas** |
