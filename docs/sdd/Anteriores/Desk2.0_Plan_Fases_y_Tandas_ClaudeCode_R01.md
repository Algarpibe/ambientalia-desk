...
**Revisión:** R01 · **Fecha:** 07/09/2026 · **Autor:** Gerencia (Alfonso García del Pino), con apoyo de Claude
**Base documental:** Documento Maestro R08.1 (§1.8, §1.9, M1.3, M2, M11, §3.2, §3.2.1, Anexos D y H) · Acta de la sesión del 03/09/2026 (Notion «Desk 2.0 03/09/2026») · Estructura de inspección física GRIMM EDM180 v9 · Decisiones de Gerencia del 07/09/2026
...
El acta del 27/08 frenó el desarrollo hasta consensuar el documento maestro y pidió, después, «trocear el proyecto por fases y prioridades». La sesión del 03/09 aportó el criterio de troceo (replicar primero el Desk 1.0), metió el módulo de informes en la Fase 1 y fijó la jerarquía del diagnóstico (fase → nivel macro → subnivel → ítem). Lo que faltaba era el paso siguiente: convertir eso en un plan que Claude Code pueda ejecutar tanda a tanda sin perder el hilo entre sesiones.
...
Objetivo: que la primera tanda funcional arranque sobre specs de lo que ya existe, no sobre memoria. Es la traducción del as-built a OpenSpec.

| Tanda | Contenido | Resultado |
|---|---|---|
| F0-01 `sdd-init` y convención | `sdd-init` sobre el repo; `openspec/config.yaml` con contexto del proyecto, `strict_tdd` según el framework de pruebas detectado, reglas por fase; `CLAUDE.md` con las reglas invariables (§4.4) | Repo preparado; Claude Code conoce las reglas sin que se le repitan |
| F0-02 Specs as-built | `sdd-explore` sobre `transitions.ts`, `transitionExec.ts`, `permissions.ts`, `estadoPorRemision.ts`, `ticketService.ts`, `avisoArea.ts`; redacción de las specs de `transitions-st`, `permissions`, `remisiones`, `zoho-sync`, `derivacion-avisos`, `trazas`, `tickets-core` **tal como están**, incluidos los seis hallazgos de la R04 marcados como «comportamiento actual, a corregir en Cx» | El Anexo H tiene su espejo en el repositorio |
| F0-03 Memoria del proyecto | Proyecto Engram; carga de las decisiones cerradas (§1.8 del documento maestro y acta 03/09) como observaciones `decision/*`; convención de `topic_key` | Cualquier sesión recupera el estado del proyecto con `mem_context` |
| F0-04 Base técnica | Pruebas mínimas del motor de transiciones (que cada transición parte y llega a los estados declarados; que `Finalizado` es el único terminal); entorno de staging sobre la VPS Hostinger con PostgreSQL; pipeline de CI que ejecuta las pruebas | Red de seguridad para tocar el motor en F1A |

Sin gates. Es trabajo de desarrollo puro y no depende de ninguna decisión pendiente.
...
Estas tandas están **escritas** desde el principio (proposal y spec con las alternativas del documento maestro), pero se ejecutan sólo cuando la sesión del viernes deja la decisión escrita. Es la aplicación literal de la «regla de la sesión» de la convocatoria del 03/09.
...
| F1C-05 | **C10** Permisos por cargo y por propietario del registro (tres niveles) | **P39** — matriz de cargos por transición; `Liberación sin factura` exclusiva del gerente comercial | M1.9.1 |
...
| F1D-03 | **Macro-fases como transiciones**: nuevas transiciones dentro de `Rev./Diagnostico` (Física → Neumática → Óptica → Electrónica, o las que resulten de P45) sobre el mismo `transitions.ts`; el equipo sin novedades recorre igualmente las macros y deja constancia | M2.1 arquitectura híbrida, P59 | **P45**: confirmar el juego de macro-fases común a todas las marcas (Johny) |
...
Fuente (documento maestro R08.1, en docs/maestro/): M2.1 «La arquitectura híbrida»,
M2.2, M2.3. Acta 03/09/2026 tema 4 («Encadenamiento pendiente»). Datos de referencia:
docs/catalogos/Inspeccion_Fisica_GRIMM_EDM180_v9.xlsx, hoja «Flujo No OK 2»
(tabla plana origen → destino, un renglón por enlace).
...
Tabla para llevar a cada sesión de los viernes. Cuando una decisión se cierra, se guarda en Engram (`decision/<clave>`) y la tanda pasa de «escrita» a «ejecutable».
...
| `decision/c5-tipo-evento` | Desambiguar «Creación de informe» | F1C-04 | 18/09 |
| `decision/c10-permisos-cargo` | Matriz cargo × transición | F1C-05 | 25/09 |
| `decision/c7-reloj-sla` | Estados que paran el reloj | F1C-06 | 25/09 |
| `decision/c6-qa-liberacion` | Etapa de QA y firmas | F1C-07 | Octubre |
| `decision/p15-p59-rutas` | Rutas abreviadas | F1C-08 | Octubre |
| `decision/falla-nueva-bloqueante` | Si el formulario de falla nueva bloquea | F1D-07 | Gustavo, sin fecha |
| `decision/p14-remisiones-entrada` | Quién escribe `remisiones_entrada` | F1E-01 | 11/09 |
| `decision/roles-validacion-informe` | Las dos personas que validan | F1E-04 | Octubre |
...
### 4.7 Qué documentos viven en el repositorio

Para que los sub-agentes de Gentle-AI tengan la fuente a mano sin depender de Notion ni del proyecto de Claude:

```
/CLAUDE.md                          reglas invariables (§4.4) y cómo se trabaja
/openspec/config.yaml               contexto, strict_tdd, reglas por fase
/openspec/specs/<capacidad>/spec.md verdad actual por capacidad
/openspec/changes/<tanda>/          proposal · specs (delta) · design · tasks
/docs/maestro/R08.1.md              documento maestro (copia versionada; se actualiza por revisión)
/docs/actas/2026-09-03.md           actas relevantes en Markdown
/docs/catalogos/*.xlsx              catálogos de inspección (GRIMM v9, Horiba)
/docs/plan/Plan_Fases_Tandas_R01.md este documento
/debt.md                            deuda técnica (ya existe)
```
...
|---|---|---|---|---|---|---|
| F0-01 | Init SDD, config y CLAUDE.md | — | §4.4 | — | S | S37 |
| F0-02 | Specs as-built de las capacidades existentes | todas las as-built | M1.3, M1.9, Anexo G | — | L | S38 |
...
*Relación con otros documentos: este plan no modifica el documento maestro; propone resolver P60 (criterio del MVP) y P62 (capa as-built → specs en el repositorio) y deja para la sesión del 11/09 los gates de §4.5. Tras esa sesión se emitirá la R02 con las decisiones incorporadas.*
