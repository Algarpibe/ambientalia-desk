...
>
>**Tercer corte del mismo día, 24/09 15:46 UTC.** Gerencia responde las **tres preguntas** que el segundo corte devolvió sobre el equipo —`equipo-nuevo-alta-en-ticket`, `edicion-datos-comerciales-equipo` y `mapa-en-la-app`— y **corrige `decision/fecha-corte`**. Las tres respuestas abren **dos** filas nuevas, no tres: el alta y la edición del equipo comparten una. **Ninguna se escribe en el §5**; el denominador pasará de **52** a **56** el día que se escriban las cuatro del día (F0-06, calendario laboral, «Alta y edición del equipo» y el mapa en la aplicación). La corrección de `fecha-corte` **no abre fila**: parte el corte en dos tiempos —14/12 operativo, independencia total en enero de 2027— y deja **dos preguntas nuevas**, `corte-licencias-plan-a` y `mapa-antes-o-despues-del-corte`, esta última porque dos decisiones del mismo día sitúan el mapa a los dos lados del corte.
...
| `decision/fecha-corte` | **DECIDIDO 24/09.** Los tickets nacen en la aplicación desde el **lunes 14 de diciembre de 2026**. **Corte en seco**: desde ese día nadie crea ni edita en Zoho Desk y ningún ticket vive en los dos sistemas. El fin de semana anterior (12–13/12) los tickets abiertos pasan a la aplicación y Zoho queda en solo lectura. Se confirma el **miércoles 9 de diciembre**; si algo falta, pasa al **lunes 21**, última fecha posible | F1F-01 | **cerrado 24/09.** ⚠️ **Es la decisión más restrictiva del corte**: el calendario del §5 llega a S53 y el corte cae en **S51**. Todo lo situado en S52–S53 queda **después**. Margen real medido el 24/09 (S39): **once semanas**, no catorce. Cuatro condiciones de confirmación, y la que hoy no tiene tanda cerrada es **responder al cliente por correo sin pasar por Zoho**. La migración es un fin de semana y **no hay copia de pruebas donde ensayarla** (decisión del 21/09). El tope del 21/12 lo fija la renovación de licencias de Zoho **PRECISADO 24/09 por Gerencia (corrección del panel, opción B), y no sustituye a lo anterior: lo parte en dos.** (1) **14/12 corte operativo** — los tickets nacen en Desk 2.0 y Zoho queda en solo lectura; hasta que exista el correo propio (**1H**), las respuestas al cliente salen del **correo corporativo** y se registran en el ticket, así que la tercera condición de confirmación **deja de bloquear el 14/12**. (2) **Independencia total en enero de 2027**, cuando **1G** (histórico) y **1H** (correo) estén verificadas: ahí se deja de consultar Zoho y se dan de baja las licencias. ⚠️ Con esto **el «corte en seco» ya no elimina la convivencia entre el 14/12 y enero**: no se ESCRIBE en Zoho (`p44` intacta) pero sí se CONSULTA. ⚠️ **Plan A con fecha: corte único el lunes 01/02/2027** si el 14/12 no conserva una semana de margen — y eso **supera el tope del 21/12** que fijaba la renovación de licencias, sin que nadie haya mirado el contrato: devuelto como `corte-licencias-plan-a`. Lo motiva la medición de la **R01.3** §D: déficit de **3,6 a 5,6 semanas**. Traza en `docs/sdd/ENTRADA.md` → E-073 **ACLARADO 24/09 por Gerencia**, sin borrar lo anterior: del examen del 9/12 salen **dos** condiciones —correo sin Zoho **e histórico completo verificado**— y pasan a enero con 1G y 1H; el examen queda en **crear y mover tickets** y **pruebas con servicios reales**. Con eso la R01.3 §D da un margen de **+1,9 a +2,2 semanas**: el plan A no se activa. Nueva pregunta devuelta: `encuesta-entre-corte-e-independencia`. Traza en E-074 |
...
| `decision/equipo-nuevo-alta-en-ticket` | **DECIDIDO 24/09.** Cuando el ticket se clasifica como «Equipo nuevo», el alta **registra el equipo en ese mismo paso** —serial, modelo del catálogo, cliente y fecha de factura obligatorios; adquisición, fin de garantía, código interno, Drive y mantenedor opcionales—, el equipo queda creado en Registro de equipos y el ticket enlazado. Si el serial ya existe **no se crea otro**: se ofrece el existente. Mantenimiento y soporte remoto **siguen exigiendo** equipo previo | **Fila nueva, talla S–M** junto con `edicion-datos-comerciales-equipo` (sin escribir en el §5) · antes de **S42** | **cerrado 24/09.** Gerencia descarta expresamente meterla en F1B-06 y da la razón: «si F1B-06 se recorta, esta pieza se mantiene» — F1B-06 es talla **L** y es la pieza que el §5 declara recortable (`:268`). Punto de partida medido el 24/09 sobre `3f30710`: el alta exige equipo existente (`apps/desk/server/services/ticketService.ts:22-24`). Los cinco campos opcionales son los que **F1B-02 acaba de construir** (archivada 23/09): el alta escribe en estructura ya hecha, no la amplía. DENOMINADOR: sube una fila el día que se escriba. Cierra E-070 |
| `decision/edicion-datos-comerciales-equipo` | **DECIDIDO 24/09.** Restricción **por área, no por cargo**: fecha de factura, fin de garantía y mantenedor sólo los cambian **Comercial y administradores**, «porque deciden si un servicio se cobra y quién puede pagarlo»; adquisición, código interno y Drive, cualquiera con sesión. Todo cambio en los seis queda **registrado con persona, fecha, valor anterior y nuevo, visible en la hoja de vida**, y **el servidor aplica la restricción, no sólo la pantalla** | **La misma fila** que `equipo-nuevo-alta-en-ticket`, que pasa a «Alta y edición del equipo», **S–M** | **cerrado 24/09.** No abre fila: el denominador sube **una sola vez** por las dos. Coherente con `c10-permisos-cargo` (23/09) —«la base sigue siendo el área»— y **no entra** en su lista corta de excepciones, así que no añade trabajo a F1C-05. Punto de partida medido: `apps/desk/server/routes/equipos.ts:73` deja hoy editar los seis campos a **cualquier sesión válida**. ⚠️ **Alcance nuevo que F1B-02 no construyó**: el historial de cambios de los seis campos — F1B-02 hizo los campos, no su registro. El mantenedor cae en la clase restringida, que es lo coherente con `titularidad-mantenedor` (21/09). Cierra E-071 |
| `decision/mapa-en-la-app` | **DECIDIDO 24/09.** Fila propia, «Mapa del blueprint en la aplicación», **antes de F1B-06**, «para que las ramas de equipo nuevo y soporte remoto se vean en la aplicación desde el día en que se construyan». **Talla condicionada a una medición**: la primera tarea es comprobar si la compilación puede producir la imagen — si puede, SVG en compilación y la pantalla lo muestra (**XS**); si no, la pantalla carga el dibujante sólo al abrirse (**S**). En los dos casos el diagrama sale del código y lo vigila la prueba que ya existe | **Fila nueva, talla XS–S por medir** (sin escribir en el §5) · precede a **F1B-06** | **cerrado 24/09.** Primera talla que Gerencia condiciona a una medición en vez de estimarla: método correcto, y el denominador sube con talla «por medir». No estrena generador — **F1A-06** (archivada 22/09) ya produce los cuatro `docs/artefactos/blueprint-*.md` con `mapaBlueprint.test.ts` (RQ-MB-06) vigilándolos; hoy **no produce SVG** (`scripts/generar-mapa-blueprint.ts`, 44 líneas, cero menciones a `svg`, medido el 24/09). Retira una marca de «próximamente» de la aplicación en uso (`apps/desk/src/components/Configuracion.tsx:118`). ⚠️ **Contradice la corrección de `fecha-corte` del mismo día**, que sitúa el mapa **después** del corte mientras F1B-06 está **antes**: devuelto como `mapa-antes-o-despues-del-corte`. Cierra E-072 · **ACLARADO 24/09 por Gerencia:** `mapa-antes-o-despues-del-corte` decidido, **después del corte**; el «antes de F1B-06» queda sin efecto (E-074) |
