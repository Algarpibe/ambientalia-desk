...
**Qué:** El comentario de `apps/desk/server/services/equipoNuevo.ts:63` llama «Validación C» al bloque que reutiliza `camposHojaDeVida`, pero la parte del mantenedor («Mantenedor no encontrado») es una comprobación de existencia de un identificador aportado tal cual, que la tabla canónica clasifica como escalón **A** (`openspec/specs/transitions-st/spec.md:774-786`). El `PATCH` de equipos, desde `edicion-comercial-equipo`, ya la trata como A. La misma guarda queda con dos etiquetas según la puerta.
