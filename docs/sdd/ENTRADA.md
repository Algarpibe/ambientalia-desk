...
**Estado:** triada
**Destino:** **propuesto F1B-06**, que es la tanda de la rama de equipo nuevo y ya creció el 24/09 con el campo «Modalidad» — pero es talla **L** y la pieza que el §5 declara recortable si hay que ganar una semana (`:268`). Si no cabe, fila propia. **Decide Gerencia:** devuelto como `equipo-nuevo-alta-en-ticket`.
...
**Estado:** triada
**Destino:** — **sin destino.** (a) es alcance nuevo sobre una tanda ya cerrada y no cabe en ninguna fila viva; (b) es una decisión de permisos que cruza `c10` («el cargo sólo restringe»). **Decide Gerencia:** devuelto como `edicion-datos-comerciales-equipo`.
...
**Estado:** triada
**Destino:** — **sin destino.** F1A-06 está archivada y F1B-09 es auditoría, no pantalla: ninguna fila viva lo contiene. **Decide Gerencia:** devuelto como `mapa-en-la-app`.
