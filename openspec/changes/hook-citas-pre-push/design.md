...
**Corregido en la tarea 3.10(a).** `entrada` es una **lectura perezosa** (`() => string`), no el texto ya
leído: `ejecutar` la invoca **sólo en modo hook**. A mano, desde una terminal, el stdin de `--sha` o de
`--generar-base` es un TTY, y una lectura ahí esperaría un EOF que no llega. El punto de entrada pasa la
función sin ejecutarla (`entrada: () => readFileSync(0, 'utf8')`); quien la ejecuta es `ejecutar`, y
sólo en la rama del modo hook.
