...
- [x] 0.1 Crear `apps/desk/server/db/transaccion.test.ts`. Sonda: mismo montaje que
...
- [x] 0.2 Ejecutar. Registrar el resultado (verde/rojo) en un comentario del fichero y en
  `apply-progress.md`: fija el plan de la Fase 1 — **Plan A** si T0 es verde, **Plan B** si es rojo.
- [x] 0.3 Si T0 es **rojo** (pg-mem no revierte de verdad): escribir en el mismo fichero un arnés
...
- [x] 1.1 RED en `transaccion.test.ts`: `enTransaccion(db, fn)` hace `BEGIN` antes de `fn`, `COMMIT`
...
- [x] 1.2 GREEN: crear `apps/desk/server/db/transaccion.ts`, `enTransaccion<T>(db, fn)`, mismo molde
  que `eliminarTicket.ts:175-189` y el bloque try/catch de `repo.ts:403-415`.
- [x] 1.3 Confirmar verde. RQ: `tickets-core` RQ-TC-16.

## Fase 2 · `repo.ts` — `opts.transaccionAbierta` — RED → GREEN

- [x] 2.1 RED en `packages/zoho-sync/src/db/repo.test.ts` (junto a `describe('createTicket
...
- [x] 2.2 GREEN: editar `repo.ts:380` (firma, 3.er parámetro `opts: { transaccionAbierta?: boolean } =
...
- [x] 2.3 Confirmar verde. RQ: `tickets-core` RQ-TC-16.

## Fase 3 · `getEquipoBySerial` + exportar `camposHojaDeVida` — RED → GREEN

- [x] 3.1 RED en `apps/desk/server/db/equipos.test.ts`: dos equipos con el mismo serial normalizado
...
- [x] 3.2 GREEN: añadir `getEquipoBySerial` al FINAL de `apps/desk/server/db/equipos.ts` (tras la
...
- [x] 3.3 `routes/equipos.ts:144`: anteponer `export` a `async function camposHojaDeVida` — única
  edición de ese fichero antes de la Fase 9, misma línea, sin desplazamiento.
- [x] 3.4 Confirmar verde. RQ: `tickets-core` RQ-TC-15.

## Fase 4 · `services/equipoNuevo.ts` — `exigirEquipoNuevo` — RED → GREEN (criterios 3-4)

- [x] 4.1 RED, al final de `ticketService.test.ts` (tras `:497`): criterio 3 — `clasificaciones:
...
- [x] 4.2 GREEN: crear `apps/desk/server/services/equipoNuevo.ts` con `exigirEquipoNuevo(db, body)`:
...
- [x] 4.3 Confirmar verde. RQ: `tickets-core` RQ-TC-04 (excepción), RQ-TC-15.

## Fase 5 · Integración en `ticketService.ts` — RED → GREEN (criterios 1, 2, 6)

- [x] 5.1 RED: criterio 1 (alta válida crea un equipo nuevo con `clientId` del ticket), criterio 2
  ...
  rojas.
- [x] 5.2 GREEN, ediciones EN SU SITIO salvo la desviación anotada arriba: `:2` retira `createTicket`
...
- [x] 5.3 Confirmar verde. RQ: `tickets-core` RQ-TC-04, RQ-TC-05, RQ-TC-15. Escenario «Mantenimiento
  sin equipo sigue rechazándose».

## Fase 6 · Atomicidad — RED → GREEN (criterio 5, RQ-TC-16)

- [x] 6.1 RED: escenario «una guarda posterior falla y no queda equipo creado» — datos de equipo
  ...
  `COMMIT`). Nace roja.
- [x] 6.2 GREEN, en `equipoNuevo.ts`: `crearTicketConEquipo(db, nuevo, clienteNombre, input)` — si el
...
- [x] 6.3 Confirmar verde. RQ: `tickets-core` RQ-TC-16.
...
- [x] 7.1 **P1** — faltan datos del equipo nuevo + OV inexistente → `422` de datos. Mutación: bajar la
  guarda 1 (`:24`) por debajo de la guarda de OV (`:37`).
- [x] 7.2 **P2** — modelo inexistente + fecha inválida → `422 'Modelo no encontrado'`. Mutación:
  validar los campos opcionales DENTRO de `exigirEquipoNuevo`, antes de resolver el modelo.
- [x] 7.3 **P3** — OV inexistente + Drive inválido → `422` de OV. Mutación: subir la validación C
  (`:89`) por encima de la guarda de OV (`:37`) — el molde IV-12.
- [x] 7.4 **P4** — serial reutilizado de otro cliente + fecha inválida → `422` equipo↔cliente.
  Mutación: subir `:89` por encima de `:59`.
- [x] 7.5 **P5** — cliente inexistente + fecha inválida (vecina de arriba) → `422 'Cliente no
  encontrado'`. Mutación: intercambiar `:87-88` con `:89`.
- [x] 7.6 **P6** — fecha inválida + OV ya usada (vecina de abajo) → `422`, no `409`. Mutación: bajar
  `:89` por debajo de `:94-98`.
- [x] 7.7 **P7** — equipo nuevo válido + OV ya usada → `409` y `COUNT(equipos)` sin cambios (criterio
  7 de `proposal.md`). Mutación: crear el equipo ANTES de la guarda de `:94`.

## Fase 8 · Cierre de mutaciones — verificación cruzada

- [x] 8.1 Confirmar que cada mutación de las Fases 0, 1 (implícita en 1.1) y 7 quedó revertida:
  `git diff --stat` vacío sobre archivos de producción tras cada reversión, antes de pasar a la
  siguiente.
- [x] 8.2 Confirmar que ninguna mutación de la Fase 7 puso en rojo un `it` DISTINTO del nombrado — si
...
- [x] 9.1 Añadir estado `equipoNuevo` (serial, `modeloId`, `fechaFacturaCompra` obligatorios;
...
- [x] 9.2 Extender la guarda de `submit` (`:183`) para aceptar el bloque completo como alternativa a
  `equipo`, y mandar `equipoNuevo` en el payload de `createTicket` (`:184-191`) cuando aplica.
- [x] 9.3 Añadir `equipoNuevo?: {...}` a `CreateTicketPayload`
...
- [x] 9.4 Comodidad de vista previa: código y asunto calculados con la serie/modelo nuevos
...
- [x] 10.1 `npm test` completo en verde; registrar el recuento total (pasadas/omitidas/ficheros) en
  `apply-progress.md`.
- [x] 10.2 `npm run typecheck` en verde, sin salida.
- [x] 10.3 `npm run lint -- --max-warnings 165` en verde; confirmar 0 warnings NUEVOS sobre la base
  preexistente (158 en el precedente más reciente de este árbol, `orden-precedencia-guardas`).
- [x] 10.4 **Barrido de citas (regla de mutación 4), OBLIGATORIO — CUATRO ficheros.** Método:
...
- [x] 10.5 `apply-progress.md`: T0 (resultado y plan usado), las 7 reversiones de P1-P7 con su diff,
