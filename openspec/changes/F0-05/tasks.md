...
- [ ] R1.0.2 `sdd-attempt acquire` sobre `main` para iniciar el intento de R1 (el ledger mide desde
      aquí; cumplida R1.0.1). Commit de partida: **`5cfd056`**.

## R1 · Fase 1 — Núcleo `cabecera.ts` (RED→GREEN) — RQ-CV-19

- [ ] R1.1.1 RED: sin bloque `---` inicial ⇒ espera invalidez, recibe `[]`.
- [ ] R1.1.2 RED: `fuera-del-plan` + `motivo: ""` ⇒ espera `campo: 'motivo'`.
- [ ] R1.1.3 RED: falta `origen_cabecera` ⇒ espera `campo: 'origen_cabecera'`.
- [ ] R1.1.4 RED: dominio cerrado (`cierra: quizá`, `origen_cabecera: heredada`) ⇒ espera el campo y su
      dominio.
- [ ] R1.1.5 RED, control del otro signo: `tanda: F9-99` con forma válida ⇒ espera `[]` (pasa aunque el
      contenido sea discutible).
- [ ] R1.1.6 GREEN: crear `apps/desk/server/citas/cabecera.ts` — parser propio de 7 claves, sin
...
- [ ] R1.2.1 Cabecera en los ocho archivados, valores de `proposal.md §4.1` (sólo se añaden líneas).
- [ ] R1.2.2 Cabecera en `F0-01`, `F0-02`, `F0-03`, valores de `proposal.md §4.2` (con los tres avisos
      ⚠️ de §4.3, sin resolverlos).
- [ ] R1.2.3 Cabecera de `orden-precedencia-guardas/proposal.md` en `main` (proposal §5.1).
...
- [ ] R1.2.5 (2b) `F0-04/proposal.md`: cabecera con `cierra: no`, `origen_cabecera: derivada-17/09`;
...
- [ ] R1.2.6 M1 (mutación, regla de mutación 2, sin rojo literal): ensuciar **EN DISCO** una copia de
...
- [ ] R1.3.1 RED: un `proposal.md` bajo `openspec/changes/archive/` sin cabecera debe salir en
      `invalidas`; hoy sale `[]` porque el alcance aún no existe.
- [ ] R1.3.2 D2, casilla con prueba propia: el alcance de cabeceras **NO** reutiliza `EXCLUSIONES`
...
- [ ] R1.3.3 GREEN: enchufar `comprobarCabeceras()` en `ejecutar()` (`cli.ts:134-179` en `995adbc`),
      lista de alcance propia (rutas trackeadas, patrón `openspec/changes/**/proposal.md`, archive
      incluido).
- [ ] R1.3.4 RED→GREEN: quinta cifra «cabeceras R-1 inválidas» en `informe.ts`, sin tocar el invariante
      de conservación de citas (D4, `detector.ts:53-56` en `995adbc`).
- [ ] R1.3.5 M4, regla de mutación 1 (posición): con cita rota y cabecera inválida a la vez en el mismo
...
- [ ] R1.3.6 `CLAUDE.md`: R-1 pasa a **siete** campos con `origen_cabecera`, dentro de la sección cuyo
      encabezado es `CLAUDE.md:419` en `5cfd056` (`:411` en `995adbc`; no se re-ancla el resto de la
      sección aquí).
- [ ] R1.3.7 Guardián de binarios (`guardianes.test.ts:16-18` en `995adbc`): los ficheros nuevos,
      trackeados y sin NUL.

## R1 · Fase 4 — Cierre de R1: barrido de anclaje, comprobación de commit y medición

- [ ] R1.4.1 Barrido de anclaje (regla de mutación 4): tomar `git diff --name-only 5cfd056`
...
- [ ] R1.4.2 H-c: clasificar por A/B/C **y reparar** a la línea que dice lo afirmado las citas de
      `CLAUDE.md:411` que hoy apuntan al encabezado y no a la frase — p.ej. «la decisión vive en
...
- [ ] R1.4.3 Citas dentro de diagramas ASCII y bloques de código: comprobar con `REVISION_RE`
      ...
      bloque, se ancla en el momento.
- [ ] R1.4.4 H-a: verificar con `git diff --cached` que el commit de R1 lleva **sólo** sus propios
...
- [ ] R1.4.5 Medir R1 en worktree aislado (`C:/dev/Desk_2_R1.023-worktrees/f0-05-r1`, sibling, nunca
...
2. Este `tasks.md` supera las 530 palabras de `sdd-tasks/SKILL.md:253`; medido en los ocho `tasks.md`
